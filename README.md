...
The Lab does not require Codex. On this VPS the default chain uses only
`codex-cli`. Claude CLI and paid HTTP providers such as OpenRouter are opt-in
with `LLM_PROVIDER_CHAIN=codex-cli,claude-cli`,
`LLM_PROVIDER_CHAIN=openrouter`, or
`LLM_PROVIDER_CHAIN=codex-cli,claude-cli,openrouter`. Installed Labs can also
run against local OpenAI-compatible endpoints such as Ollama. The invariant is
the artifact
...
Alpha. The physics domain ports the original research lab that runs at
[lab.d-nd.com](https://lab.d-nd.com). Finance, BTC and research-radar are
active hardening domains; draft Labs that do not declare a useful autonomous
state are removed from the visible fleet until regenerated by the meta-lab.
